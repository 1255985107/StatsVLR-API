...
async function getPlayerById(id) {
  ...
  const $ = await request({
    uri: `${vlrgg_url}/player/${id}/?timespan=all`,
    transform: (body) => cheerio.load(body),
  });
  ...
  $(".player-summary-container-1 .wf-card.mod-table.mod-dark table tbody tr").each((i, el) => {
    ...
    const useRate = $(el).find("td").eq(1).text().trim();
    const roundsPlayed = $(el).find("td").eq(2).text().trim();
    const Rating = $(el).find("td").eq(3).text().trim();
    const averageCombatScore = $(el).find("td").eq(4).text().trim();
    const KAST = $(el).find("td").eq(7).text().trim();
